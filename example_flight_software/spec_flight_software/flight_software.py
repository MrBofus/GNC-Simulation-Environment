...
from example_flight_software.spec_flight_software.local_gnc_library import *
...
class schedulerApp():
    ...
    def _iterate(self):
        if self.systemClock == 1:
            self.mode = 'detumble'
            self.quaternion_error_previous = np.array([0.0, 0.0, 0.0, 0.0])
    ...
    def _run_control(self):
        if self.mode == 'detumble':
            ...
            q_error, mt_command = magneticController( self.m.measurements['angularRate'], self.m.measurements['quaternion'], 
                                                      q_setpoint, [0.0, 0.0, 0.0], self.quaternion_error_previous,
                                                      self._p['smc_kp'], self._p['smc_ki'], self._p['smc_kd'], self._p['smc_sigma'], self._p['smc_order'],
                                                      self.m.measurements['bField'] )
            
            self.quaternion_error_previous += q_error
